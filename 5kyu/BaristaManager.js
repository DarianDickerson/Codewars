...
function barista(coffees, cMach){
    coffees = coffees.sort((a,b)=>(a-b)).filter(joe=>joe>0)
    
    let lines = new Array(cMach).fill(null).map(a=>[])
    
    for(let i=0; i<coffees.length; i++){
      lines[i%cMach].push(coffees[i])
    }
    
    return lines.reduce(function(acc,j){
      ...
      return acc + sum.reduce((ac,n)=>(ac+=n),0) 
    },0)
}